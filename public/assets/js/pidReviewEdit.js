...
var pid = new Vue({
    ...
    methods: {
        ...
        salvarPid: function(ev) {
            ...
            self.$set('pid.session_email', jQuery("#session_email").val());
            self.$set('pid.session_pass', jQuery("#session_pass").val());

            self.$http.post('/revisao/pid/store', self.pid, function (response){
                self.$set('pid', response);
                jQuery('#loading').modal('hide');
                self.alerta(false, {msg:['Enviado com sucesso!']})
                self.$set('sent', true);
    ...
    ready: function() {
        ...
        if(idPid != '') {
            ...
            /*Busca os dados do PID no BD e add ao formulario*/
            url = '/revisao/pid/'+idPid+'/show';
